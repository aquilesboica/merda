...
import { ConnectionState, TranscriptionEntry } from './types.ts';
import { SYSTEM_INSTRUCTION, MODEL_NAME } from './constants.ts';
...
export default function App() {
  ...
  const sendTextMessage = (text: string) => {
    if (sessionRef.current && connectionState === ConnectionState.CONNECTED) {
      // In Gemini Live sessions, text can be sent via content parts
  ...
  const startConversation = async () => {
    try {
      ...
      // Ensure we use the most up-to-date API key by creating the instance right before connection
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            // Handle raw PCM audio output from Gemini
            ...
            // When a turn is complete, flush transcription to state
            ...
            // Handle session interruptions (e.g., user speaking over the AI)
